...
export interface OpenAIVisionServiceOptions {
  apiKey?: string;
...
type NormalizedScale = NonNullable<VisionAnalysisResult["scale"]>;

export class OpenAIVisionService {
  private readonly logger = createScopedLogger("OpenAIVisionService");
  ...
  constructor(options: OpenAIVisionServiceOptions) {
    ...
    this.allowMockFallback = Boolean(options.allowMockFallback);
    ...
    this.retryDelayMs =
      ...
      parseInt(process.env.OPENAI_RETRY_DELAY_MS || "1000", 10);
  }
  ...
  async analyzePlanImage(
    ...
    if (this.nodeEnv !== "production") {
      this.logger.log(
        `Analyzing plan image with OpenAI Vision (disciplines: ${disciplines.join(
          ","
...
Do not add prose, markdown, or explanations beyond the JSON object.`,
  ...
  private createAnalysisPrompt(
    ...
    const disciplineDescription = disciplines
      .map((d) => disciplineMap[d as keyof typeof disciplineMap] || d)
      .join(", ");
    const targetDescription = targets
      .map((t) => targetMap[t as keyof typeof targetMap] || t)
      .join(", ");
...
DISCIPLINES TO ANALYZE: ${disciplineDescription}

EXTRACTION TARGETS: ${targetDescription}
...
- Return valid JSON only - but JSON with empty arrays for visible elements is INCOMPLETE`;
  ...
  private validatePipes(items: any[], shouldInclude: boolean): VisionAnalysisResult["pipes"] {
    ...
    const validatedPipes = items.map((item, index) => {
      ...
      if (!item.length || length === undefined) {
        appLogger.warn(
          `[OpenAIVisionService] Pipe ${item.id || index} missing length: raw=${JSON.stringify(item.length)} → parsed=${length}`
        );
      }
    ...
    if (withLengths.length < validatedPipes.length) {
      appLogger.warn(
        `[OpenAIVisionService] ${validatedPipes.length - withLengths.length} of ${validatedPipes.length} pipes missing length values`
      );
    }
  ...
  private validateDucts(items: any[], shouldInclude: boolean): VisionAnalysisResult["ducts"] {
    ...
    return items.map((item, index) => {
      ...
      if (item.length !== undefined && length === undefined) {
        appLogger.warn(
          `[OpenAIVisionService] Failed to parse duct length: raw="${item.length}" → parsed=undefined`
        );
      }
  ...
  private validateAndFilterWalls(
    ...
    for (const wall of walls) {
      ...
      const isColumnBySize =
        typeof length === "number" &&
        Array.isArray(wall.polyline) &&
        wall.polyline.length === 4 &&
        length < 2.0; // Small square-like shape < 2ft is likely a column
      const isColumn =
  ...
  private normalizeScale(scale: any): NormalizedScale {
...
Return as structured JSON.`,
      ...
      this.logger.warn("OpenAI text analysis failed:", this.getErrorMessage(error));
      return {};
    }
  }

  async detectScale(imageBuffer: Buffer): Promise<NormalizedScale> {
    try {
      const base64Image = imageBuffer.toString("base64");
...
Return ONLY a JSON object with:
{
  ...
  "method": "titleblock/dimensions/reference"
      ...
      this.logger.warn("OpenAI scale detection failed:", this.getErrorMessage(error));
