...
import { SheetData, PartitionTypeDefinition, SpaceDefinition, WallRunSegment } from '../../types/vision';
...
export class WallRunExtractionService {
  private readonly logger = scopedLogger('WallRunExtractionService');
  private readonly openai?: OpenAI;
  private readonly model: string;
  private readonly partitionContextBudget: number;

  constructor() {
    this.model = config.openAiWallModel || config.openAiModel || 'gpt-4o-mini';
    this.partitionContextBudget = config.wallPartitionContextLimit ?? 6000;
  ...
  async extractWallRuns(
    ...
    for (const sheet of floorPlanSheets) {
      ...
      try {
        ...
        this.logger.warn(
          `Wall run extraction failed for sheet ${sheet.name || sheet.index}: ${message}`,
        );
      }
    }
  ...
  private async extractFromSheet(
    ...
    const response = await this.openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You map wall segments on floor plan images. Return JSON arrays only.',
        },
        {
          role: 'user',
          content: [
