...
import { SheetData, SpaceDefinition, SpaceCategory } from '../../types/vision';
...
export class SpaceExtractionService {
  private readonly logger = scopedLogger('SpaceExtractionService');
  private readonly openai?: OpenAI;
  private readonly model: string;
  private readonly textBudget: number;

  constructor() {
    this.model = config.openAiSpaceModel || config.openAiModel || 'gpt-4o-mini';
    this.textBudget = config.spaceTextLimit ?? 6000;
  ...
  async extractSpaces(sheets: SheetData[]): Promise<SpaceDefinition[]> {
    ...
    const targetSheets = sheets.filter((sheet) => {
      ...
      const hasTextData = sheet.content?.textData && sheet.content.textData.length > 100;
    ...
    for (const sheet of targetSheets) {
      try {
        ...
        this.logger.error(
          `Space extraction failed for sheet ${sheet.name || sheet.index}: ${message}`,
        );
      }
    }
  ...
  private async extractFromSheet(sheet: SheetData) {
    ...
    const instructions =
      `You are extracting logical spaces (rooms or zones) from a ${
        hasRasterData ? 'plan' : 'text-based plan document'
      }.\n` +
