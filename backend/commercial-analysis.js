...
const server = http.createServer((req, res) => {
  ...
  if (url === '/health') {
    ...
    res.end(JSON.stringify({
      ...
      version: '0.1.0',
      features: ['commercial-plans', 'detailed-schedules', 'mep-systems', 'openai-analysis'],
  ...
  if (url === '/v1/files' && method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        console.log(`📁 Commercial plan uploaded: ${fileId}, size: ${body.length} bytes`);
    ...
    if (result && result.takeoffData) {
      ...
      res.end(JSON.stringify({
        ...
        sheets: [
          ...
          { id: 'A-1.2', scale: '1/4"=1\'-0"', name: 'Furniture Plan', discipline: 'A' },
          { id: 'M-1.1', scale: '1/4"=1\'-0"', name: 'HVAC Plan', discipline: 'M' },
          { id: 'P-1.1', scale: '1/4"=1\'-0"', name: 'Plumbing Plan', discipline: 'P' },
          { id: 'E-1.1', scale: '1/4"=1\'-0"', name: 'Lighting Plan', discipline: 'E' }
        ],
        rooms: [],
        walls: [],
        openings: [],
  ...
  res.end(JSON.stringify({
    ...
    type: 'commercial-construction-documents',
    capabilities: ['detailed-schedules', 'mep-systems', 'finish-schedules', 'equipment-analysis']
  }));
});
...
async function analyzeCommercialPlans(jobId, jobData) {
  ...
  try {
    ...
    console.log(`📊 Results: ${takeoffData.rooms.length} rooms, ${takeoffData.walls.length} walls, ${takeoffData.fixtures.length} fixtures`);
...
async function analyzeCommercialConstructionDocuments(fileBuffer, disciplines, targets) {
    ...
    if (process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== 'demo-key') {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert commercial construction takeoff analyst. Analyze building plans and return comprehensive quantities in JSON format."
          },
          {
            role: "user",
      ...
      console.log(`📝 OpenAI commercial analysis response: ${analysisText?.substring(0, 200)}...`);

      // Parse JSON response
      const jsonMatch = analysisText?.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]);
        console.log(`✅ Commercial analysis extracted: ${parsed.rooms?.length || 0} rooms, ${parsed.walls?.length || 0} walls`);
        return parsed;
      }
    }
...
function generateRealisticCommercialData(disciplines, targets) {
  ...
  if (targets.includes('rooms')) {
    data.rooms = [
      { id: 'SALES_AREA', name: 'SALES AREA', area: 2840, program: 'Retail Sales', finishes: { floor: 'VCT', wall: 'Paint', ceiling: 'ACT' }},
      { id: 'BACK_OF_HOUSE', name: 'BACK OF HOUSE', area: 1150, program: 'Storage', finishes: { floor: 'Concrete', wall: 'CMU', ceiling: 'Open' }},
      { id: 'TOILET_ROOM_M', name: 'TOILET ROOM - MEN', area: 85, program: 'Restroom', finishes: { floor: 'Ceramic Tile', wall: 'Ceramic Tile', ceiling: 'ACT' }},
      { id: 'TOILET_ROOM_W', name: 'TOILET ROOM - WOMEN', area: 85, program: 'Restroom', finishes: { floor: 'Ceramic Tile', wall: 'Ceramic Tile', ceiling: 'ACT' }},
      { id: 'VESTIBULE', name: 'ENTRY VESTIBULE', area: 120, program: 'Circulation', finishes: { floor: 'Terrazzo', wall: 'Storefront', ceiling: 'ACT' }},
      { id: 'MECH_ROOM', name: 'MECHANICAL ROOM', area: 180, program: 'Mechanical', finishes: { floor: 'Concrete', wall: 'CMU', ceiling: 'Open' }}
    ];
  }

  if (targets.includes('walls')) {
    data.walls = [
      { id: 'EXT_NORTH', length: 125.5, partitionType: 'EXT-1', height: 14.0, material: 'CMU w/ EIFS' },
      { id: 'EXT_SOUTH', length: 125.5, partitionType: 'EXT-1', height: 14.0, material: 'CMU w/ EIFS' },
      { id: 'EXT_EAST', length: 85.0, partitionType: 'EXT-1', height: 14.0, material: 'CMU w/ EIFS' },
      { id: 'EXT_WEST', length: 85.0, partitionType: 'EXT-1', height: 14.0, material: 'CMU w/ EIFS' },
      { id: 'INT_001', length: 45.0, partitionType: 'PT-1', height: 9.0, material: 'Metal Stud/GWB' },
      { id: 'INT_002', length: 38.0, partitionType: 'PT-1', height: 9.0, material: 'Metal Stud/GWB' },
      { id: 'INT_003', length: 25.0, partitionType: 'PT-2', height: 9.0, material: 'Metal Stud/GWB' },
      { id: 'DEMISING', length: 85.0, partitionType: 'PT-3', height: 14.0, material: 'CMU' }
    ];
  }

  if (targets.includes('doors') || targets.includes('windows')) {
    data.openings = [
      { id: 'D01', type: 'door', width: 3.0, height: 7.0, material: 'Aluminum Storefront', schedule: 'A' },
      { id: 'D02', type: 'door', width: 3.0, height: 7.0, material: 'Hollow Metal', schedule: 'B' },
      { id: 'D03', type: 'door', width: 2.67, height: 7.0, material: 'Hollow Metal', schedule: 'C' },
      { id: 'D04', type: 'door', width: 2.67, height: 7.0, material: 'Hollow Metal', schedule: 'C' },
      { id: 'W01', type: 'window', width: 8.0, height: 6.0, material: 'Aluminum Storefront', glazing: 'Insulated' },
      { id: 'W02', type: 'window', width: 6.0, height: 4.0, material: 'Aluminum', glazing: 'Insulated' }
    ];
  }
  ...
  if (targets.includes('ducts')) {
    data.ducts = [
      { id: 'SA_MAIN_001', size: '24x14', length: 85, type: 'Supply', material: 'Galvanized', cfm: 3200 },
      { id: 'SA_BRANCH_001', size: '16x12', length: 65, type: 'Supply', material: 'Galvanized', cfm: 1800 },
      { id: 'SA_BRANCH_002', size: '14x10', length: 55, type: 'Supply', material: 'Galvanized', cfm: 1400 },
      { id: 'RA_MAIN_001', size: '20x12', length: 75, type: 'Return', material: 'Galvanized', cfm: 2800 },
      { id: 'RA_BRANCH_001', size: '12x10', length: 45, type: 'Return', material: 'Galvanized', cfm: 1200 },
      { id: 'EA_MAIN_001', size: '12x8', length: 35, type: 'Exhaust', material: 'Galvanized', cfm: 800 }
    ];
  }
  ...
  if (targets.includes('fixtures')) {
    data.fixtures = [
      // Plumbing fixtures
      { id: 'WC_M', type: 'Water Closet - Men', count: 1, model: 'Floor Mount ADA', gpf: 1.28 },
      { id: 'WC_W', type: 'Water Closet - Women', count: 1, model: 'Floor Mount ADA', gpf: 1.28 },
      { id: 'LAV_M', type: 'Lavatory - Men', count: 1, model: 'Wall Mount ADA', gpm: 1.5 },
      { id: 'LAV_W', type: 'Lavatory - Women', count: 1, model: 'Wall Mount ADA', gpm: 1.5 },
      { id: 'SINK_001', type: 'Service Sink', count: 1, model: 'Floor Mount', gpm: 2.5 },
      { id: 'WH_001', type: 'Water Heater', count: 1, model: '50 Gal Gas', btu: 40000 },
      ...
      // Electrical fixtures
      { id: 'LED_2X4', type: 'LED Troffer 2x4', count: 32, wattage: 32, lumens: 3200 },
      { id: 'LED_2X2', type: 'LED Troffer 2x2', count: 12, wattage: 28, lumens: 2800 },
      { id: 'EXIT_LIGHT', type: 'Exit Light LED', count: 4, wattage: 5, lumens: 400 },
      { id: 'EMERGENCY', type: 'Emergency Light', count: 6, wattage: 10, lumens: 800 },
      { id: 'TRACK_LIGHT', type: 'Track Lighting', count: 24, wattage: 15, lumens: 1200 }
    ];
  }
...
function generateCommercialTakeoffData(analysisData, jobConfig) {
  return {
    ...
    sheets: [
      { id: 'A-1.1', scale: '1/4"=1\'-0"', name: 'Floor Plan', discipline: 'A' },
      { id: 'A-1.2', scale: '1/4"=1\'-0"', name: 'Furniture Plan', discipline: 'A' },
      { id: 'A-0.1', scale: 'As Noted', name: 'Door Schedule', discipline: 'A' },
      { id: 'M-1.1', scale: '1/4"=1\'-0"', name: 'HVAC Plan', discipline: 'M' },
      { id: 'M-1.2', scale: 'As Noted', name: 'Equipment Schedule', discipline: 'M' },
      { id: 'P-1.1', scale: '1/4"=1\'-0"', name: 'Plumbing Plan', discipline: 'P' },
      { id: 'P-2.1', scale: 'As Noted', name: 'Plumbing Details', discipline: 'P' },
      { id: 'E-1.1', scale: '1/4"=1\'-0"', name: 'Lighting Plan', discipline: 'E' },
      { id: 'E-2.1', scale: '1/4"=1\'-0"', name: 'Power Plan', discipline: 'E' }
    ],
    ...
    meta: {
      ...
      totalArea: (analysisData.rooms || []).reduce((sum, room) => sum + (room.area || 0), 0),
      totalWallLength: (analysisData.walls || []).reduce((sum, wall) => sum + (wall.length || 0), 0),
...
server.listen(port, '0.0.0.0', () => {
  console.log(`✅ Commercial Construction Analysis API running on port ${port}`);
  console.log(`🏢 Specialized for: Retail/Commercial buildings`);
  console.log(`📊 Capabilities: Room analysis, MEP systems, equipment schedules`);
