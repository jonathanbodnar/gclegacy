...
import { RoomCeilingHeight } from './ceiling-height-extraction.service';
...
export class FinalDataFusionService {
  ...
  constructor(configService: ConfigService) {
    this.defaultDpi = parseInt(configService.get<string>('PDF_RENDER_DPI') || '220', 10);
  }
  ...
  private combineRooms(
    ...
    for (const schedule of schedules) {
      ...
      entry.room.ceiling_finish_code = schedule.ceiling_finish_code ?? entry.room.ceiling_finish_code;
  ...
  private convertUsingScaleRatio(
    ...
    if (!Number.isFinite(inchesPerPixel)) {
      return undefined;
    }
  ...
  private parseScaleNote(note: string): { planInches: number; realFeet: number } | undefined {
    if (!note) return undefined;
    const cleaned = note.replace(/\s+/g, '').toLowerCase();

    // Match fraction inch = feet-inches e.g. 1/4"=1'-0"
    const fracMatch = cleaned.match(/(\d+)\/(\d+)"=?(\d+)'?-?(\d+)?("?)/);
    if (fracMatch) {
      const numerator = parseFloat(fracMatch[1]);
      const denominator = parseFloat(fracMatch[2]);
      const planInches = numerator / denominator;
      const feet = parseFloat(fracMatch[3]) || 0;
      const inches = parseFloat(fracMatch[4]) || 0;
      const realFeet = feet + inches / 12;
      if (planInches > 0 && realFeet > 0) {
        return { planInches, realFeet };
      }
    }

    // Match decimal inch = feet e.g. 1"=8'-0"
    const decMatch = cleaned.match(/([\d.]+)"=?(\d+)'?-?(\d+)?/);
    if (decMatch) {
      const planInches = parseFloat(decMatch[1]);
      const feet = parseFloat(decMatch[2]) || 0;
      const inches = parseFloat(decMatch[3]) || 0;
      const realFeet = feet + inches / 12;
      if (planInches > 0 && realFeet > 0) {
        return { planInches, realFeet };
      }
