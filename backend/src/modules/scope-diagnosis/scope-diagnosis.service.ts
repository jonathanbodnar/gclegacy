...
export class ScopeDiagnosisService {
  ...
  private buildHeuristicDiagnosis(
    ...
    if (!pipeLength && input.targets.includes("pipes")) {
      noteSegments.push(
        "No piping runs detected - verify plumbing scope manually."
      );
    }
    if (!verticalSystems?.riserCount) {
      noteSegments.push(
        "Vertical risers not detected; confirm elevations and riser diagrams."
      );
    }
...
Use CSI MasterFormat terminology where possible.`;
  ...
  private mergeResults(
    ...
    if (
      !Array.isArray(merged.csiDivisions) ||
      merged.csiDivisions.length === 0
    ) {
    ...
    if (!Array.isArray(merged.notes)) {
      ...
      merged.notes = Array.from(
        new Set([...(heuristic.notes || []), ...merged.notes])
      );
    }
  ...
  private deriveAssemblySummary(
    ...
    const roomCount = features.filter(
      (feature) => feature.type === "ROOM"
    ).length;
    if (roomCount > 0) {
      const totalArea =
        features
  ...
  private buildVerticalSummary(
    ...
    if (pipeLength > 0 && !vertical.riserCount) {
      vertical.notes?.push(
        "Piping present without riser diagrams; consider riser assumptions."
      );
    }
  ...
  private buildFittingEstimates(features: any[]): FittingEstimate[] {
    ...
    for (const feature of features) {
      ...
      if (!pipeGroups.has(service)) {
        pipeGroups.set(service, {
          length: 0,
          count: 0,
          label: `${service} piping`,
        });
      }
