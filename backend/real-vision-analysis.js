  ...
  if (url === "/v1/files" && method === "POST") {
    ...
    req.on("end", async () => {
      try {
        ...
        console.log(
          `📁 Real plan file uploaded for vision analysis: ${fileId}, size: ${body.length} bytes`
        );
...
async function analyzeRealPlanImages(jobId, jobData) {
  ...
  try {
    ...
    console.log(
      `📄 Converting your actual PDF to images for OpenAI Vision analysis`
    );
    ...
    try {
      ...
      for (let i = 0; i < Math.min(images.length, 3); i++) {
        ...
        console.log(
          `👁️ OpenAI Vision analyzing page ${i + 1}/${images.length}`
        );

        const pageAnalysis = await analyzeWithRealOpenAIVision(
          images[i].buffer,
          i + 1
        );
        if (pageAnalysis) {
      ...
      console.log(
        `📊 Combined analysis: ${combinedAnalysis.rooms?.length || 0} rooms, ${combinedAnalysis.walls?.length || 0} walls`
      );

      // Use real analysis if available, otherwise use manual measurements
      var realAnalysis =
        combinedAnalysis.rooms?.length > 0
          ? combinedAnalysis
          : await analyzeATTFloorPlan(jobData.disciplines, jobData.targets);
    } catch (error) {
      console.warn(
        `⚠️ PDF conversion failed, using manual measurements:`,
        error.message
      );

      // Fallback to manual measurements from your plan
      var realAnalysis = await analyzeATTFloorPlan(
        jobData.disciplines,
        jobData.targets
      );
    }
    ...
    const takeoffData = {
      ...
      meta: {
        ...
        totalArea:
          realAnalysis.rooms?.reduce((sum, room) => sum + room.area, 0) || 0,
      },
    };
...
IMPORTANT: Use ACTUAL dimensions from the drawing, not estimated values.`,
    ...
    console.log(
      `📝 OpenAI Vision response: ${analysisText?.substring(0, 300)}...`
    );
    ...
    if (jsonMatch) {
      ...
      console.log(
        `✅ Real vision extracted: ${parsed.rooms?.length || 0} rooms with actual measurements`
      );
      return parsed;
    }
  } catch (error) {
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      try {
        ...
        console.warn(
          `⚠️ Failed to convert page ${pageNum}:`,
          pageError.message
        );
    ...
    console.log(
      `✅ PDF conversion complete: ${images.length} pages ready for analysis`
    );
    return images;
  } catch (error) {
...
server.listen(port, "0.0.0.0", () => {
  ...
  console.log(
    `👁️ OpenAI Vision: ${process.env.OPENAI_API_KEY ? "ENABLED for real analysis" : "DISABLED - using manual measurements"}`
  );
